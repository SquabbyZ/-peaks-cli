...
- `route`：判断这次改动该走什么路线，输出路线图。
- `autonomous`：生成全自动治理链路预览。
- `tech-plan`：把技术目标拆成可审阅的 RD 范围。
- `tech-status`：查看技术 artifact / approval 状态。
- `swarm-plan`：把已批准的 RD 范围拆成并行执行计划。
- `minimax-worker`：把一个明确的小任务交给 MiniMax worker 执行，并返回给顶级模型审查的交接结果。
...
用途：帮助你决定是否应该调用外部 skills、MCP、hooks、agent browser、OpenSpec 等能力。
